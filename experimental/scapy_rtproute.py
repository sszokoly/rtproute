...
## Comment: does not work as expected when device has multiple route tables
##############################################################################
...
def rtproute(host, port=33434, src_addr=None, sport=5060, device=None,
             ...
             payload_type=214, quiet=False, seqno=1, ssrc=3735928559,
    ...
    if device and not src_addr:
        try:
            addrs = netifaces.ifaddresses(device)
            src_addr = addrs[netifaces.AF_INET][0]['addr']
        except:
            pass
    ...
    for ttl in range(first_ttl, max_ttl + 1):
        ...
        # Ethernet (14) + IP(20) + UDP(8) + RTP(12) + payload = packetlen
        rtp_payload_size = max(0, packetlen - 14 - 20 - 8 - 12)
        ...
        reply = sr1(pkt, timeout=max_wait, verbose=0, iface=device)
...
if __name__ == '__main__':
    ...
    sys.argv.extend(['-m', '1', '-i', 'B1', '8.8.8.8'])
    ...
    optional.add_argument(
        ...
        default=3735928559,
        metavar="ssrc",
        help="Set the RTP SSRC, Default is 3735928559 (0xdeadbeef)",
    )
    optional.add_argument(
        "--timestamp",
        dest="timestamp",
    ...
    positional.add_argument(
        ...
        type=int,
        default=214,
        help=f"The full packet length, Default is 214 (G711/ptime 20)",
    )
